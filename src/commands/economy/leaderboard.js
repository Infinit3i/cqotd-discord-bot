...
async function handleLeaderboard(client, interaction) {
  ...
  try {
    ...
    // Generate a complete list of users with scores greater than 0
    const fullLeaderboard = members
      ...
      .filter((entry) => entry.score > 0); // Only include users with scores > 0
    ...
    if (leaderboard) {
      ...
      interaction.editReply("🏆 No users with points yet. Be the first to score!");
    }
  } catch (error) {
